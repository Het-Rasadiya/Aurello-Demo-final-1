...
theme.swatchCard = (function () {
  function updateCard(element) {
    ...
    // Update links to point to the selected variant
    if (newUrl) {
      gridItem.find('.product-card__image, .product-card__name').attr('href', newUrl);
    }

    // Update price representation
    var $price = gridItem.find('.product-card__price');
    if ($price.length && price) {
      if (comparePrice && Number(comparePrice) > Number(price)) {
        $price.addClass('product-card__pricesale');
        var htmlComparePrice = '<s class="product-card__regular-price"><span class="money">' + theme.Currency.formatMoney(comparePrice, theme.moneyFormat) + '</span></s>';
      } else {
        $price.removeClass('product-card__pricesale');
        var htmlComparePrice = '';
      }
      var htmlPrice = '<span class="money">' + theme.Currency.formatMoney(price, theme.moneyFormat) + '</span> ' + htmlComparePrice;
